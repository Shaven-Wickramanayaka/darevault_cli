...
async fn random_dare(Path(id): Path<i64>) -> Result<Json<Value>,APIError>{
    ...
    ").await.unwrap();
    let row = sqlx::query_as::<_,DareRetrived>("SELECT * FROM dares ORDER BY RANDOM() LIMIT 1").bind(id).fetch_optional(&connection).await.map_err(|_| APIError::InternalError)?;
    match row {
        Some(r) =>  Ok( Json( json!({"id": id,"dare": r.dare, "created_by": r.created_by}) )),
        None => Err(APIError::NotFound),
    }
